...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        self.client = OpenAI(
            base_url="http://10.174.138.18:8000/v1",
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            # 获取整合了genrm的结果的batch
            # test_batch = test_batch.union(result)
            # evaluate using reward_function
            result = self.val_reward_fn(test_batch, return_dict=True)
            reward_tensor = result["reward_tensor"]
            scores = reward_tensor.sum(-1).cpu().tolist()
            response_length, max_response_length = compute_val_response_length(test_batch)
            sample_response_length.extend(response_length.tolist())
        ...
        reward_extra_infos_dict.update({"response_length": sample_response_length})
        if val_data_dir:
        ...
        # sample_response_length = torch.tensor(sample_response_length)
        # length_metrics = {
        #     "val-core/val_response_length/mean": torch.mean(sample_response_length).detach().item(),
        #     "val-core/val_response_length/max": torch.max(sample_response_length).detach().item(),
        #     "val-core/val_response_length/min": torch.min(sample_response_length).detach().item(),
        #     "val-core/max_response_length": max_response_length,
        #     "val-core/val_response_length/clip_ratio": torch.mean(torch.eq(sample_response_length, max_response_length).float()).detach().item(),
        # }
        # metric_dict.update(length_metrics)
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                # ####### Add For table logging SRART #############
                # # Adopted from trl/grpo_trainer.py
                # import pandas as pd
                # import wandb
                # prompt_list, response_list, sequence_scores, ground_truth_list = get_table_data(batch, self.tokenizer, 
                #                                                                                 self.config.trainer.experiment_name, 
                #                                                                                 self.global_steps)
                # table = {
                #     "step": [self.global_steps] * len(prompt_list),
                #     "prompt": prompt_list,
                #     "ground truth": ground_truth_list,
                #     "completion": response_list,
                #     "reward": sequence_scores,
                # }
                # df = pd.DataFrame(table)

                # wandb.log({"completions": wandb.Table(dataframe=df)})
                # ####### Add For table logging END #############
