...
def compute_val_response_length(batch: DataProto) -> Dict[str, Any]:
    response_info = _compute_response_info(batch)
    response_length = response_info["response_length"]
    max_response_length = batch.batch["responses"].shape[-1]
    return response_length, max_response_length
...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> Dict[str, Any]:
    ...
    score_1_mask = sequence_score == 1
    score_0_mask = sequence_score == 0
    print(response_length[score_1_mask])
    metrics = {
        # score
    ...
    if score_1_mask.any():
        score_1_lengths = response_length[score_1_mask]
        metrics.update({
            "response_length/mean_adv>0": torch.mean(response_length[score_1_mask]).detach().item(),
            "response_length/max_adv>0": torch.max(response_length[score_1_mask]).detach().item(),
            "response_length/min_adv>0": torch.min(response_length[score_1_mask]).detach().item(),
            "response_length/std_adv>0": torch.std(response_length[score_1_mask], unbiased=False).detach().item(),
        })
    else:
        metrics.update({
            "response_length/mean_adv>0": -100,
            "response_length/max_adv>0": -100,
            "response_length/min_adv>0": -100,
            "response_length/std_adv>0": -100,
        })

    if score_0_mask.any():
        score_0_lengths = response_length[score_0_mask]
        metrics.update({
            "response_length/mean_adv<0": torch.mean(response_length[score_0_mask]).detach().item(),
            "response_length/max_adv<0": torch.max(response_length[score_0_mask]).detach().item(),
            "response_length/min_adv<0": torch.min(response_length[score_0_mask]).detach().item(),
            "response_length/std_adv<0": torch.std(response_length[score_0_mask], unbiased=False).detach().item(),
        })
    else:
        metrics.update({
            "response_length/mean_adv<0": -100,
...
def get_table_data(batch, tokenizer, experiment_name, global_step):
    ...
    prompt_list = [tokenizer.decode(ids) for ids in valid_prompt_ids]
    response_list = [tokenizer.decode(ids) for ids in valid_response_ids]

    # 计算 sequence scores
    sequence_scores = token_level_scores_batch.sum(dim=-1).tolist()
    
    return prompt_list, response_list, sequence_scores, ground_truth_list
