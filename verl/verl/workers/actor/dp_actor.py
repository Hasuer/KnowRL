...
class DataParallelPPOActor(BasePPOActor):
    ...
    def update_policy(self, data: DataProto):
        ...
        # Select required keys from input data
        select_keys = ["responses", "input_ids", "attention_mask", "position_ids", "old_log_probs", "advantages"]
        if multi_turn:
            select_keys.append("loss_mask")  # Add loss mask for multi-turn conversations
        if self.config.use_kl_loss:
            select_keys.append("ref_log_prob")  # Add reference log probs if using KL loss
        batch = data.select(batch_keys=select_keys).batch
        has_multi_modal_inputs = "multi_modal_inputs" in data.non_tensor_batch.keys()

        # Prepare mini-batches for PPO training
        if has_multi_modal_inputs:
            ...
            for batch_idx, data in enumerate(dataloader):
                ...
                if has_multi_modal_inputs:
                    ...
                    if self.config.use_dynamic_bsz:
                        # Handle dynamic batch size for multi-modal inputs
                        all_multi_modal_inputs_list = data.non_tensor_batch["multi_modal_inputs"]
                        ...
                        # Standard mini-batch splitting for multi-modal inputs
                        self.gradient_accumulation = self.config.ppo_mini_batch_size // self.config.ppo_micro_batch_size_per_gpu
                        num_micro_batches = mini_batch.batch.batch_size[0] // self.config.ppo_micro_batch_size_per_gpu
                        micro_batches = data.select(select_keys, non_tensor_select_keys).chunk(num_micro_batches)
                elif self.config.use_dynamic_bsz:
                    # Handle dynamic batch size for text-only inputs
                    max_token_len = self.config.ppo_max_token_len_per_gpu * self.ulysses_sequence_parallel_size
                    micro_batches, _ = rearrange_micro_batches(batch=mini_batch, max_token_len=max_token_len)
                else:
                    # Standard mini-batch splitting for text-only inputs
                    self.gradient_accumulation = self.config.ppo_mini_batch_size // self.config.ppo_micro_batch_size_per_gpu
                ...
                for data in micro_batches:
                    ...
                    if multi_turn:
                        response_mask = data["loss_mask"][:, -response_length:]  # Use loss mask for multi-turn
                    else:
                        response_mask = attention_mask[:, -response_length:]  # Use attention mask for single-turn
                    ...
                    # Forward pass to get entropy and log probabilities
                    ...
                    # Compute policy loss based on selected mode
                    ...
                    # Add KL divergence penalty if enabled
                    if self.config.use_kl_loss:
                    ...
                    if self.config.use_dynamic_bsz:
                        loss = policy_loss * (len(data) / self.config.ppo_mini_batch_size)  # Scale by relative batch size
                    else:
                        loss = policy_loss / self.gradient_accumulation  # Normalize by gradient accumulation steps
                    loss.backward()
                    ...
                    data = {
                        ...
                        "actor/pg_clipfrac_lower": pg_clipfrac_lower.detach().cpu().tolist(),
                    }
