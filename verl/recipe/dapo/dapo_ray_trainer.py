...
class RayDAPOTrainer(RayPPOTrainer):
    ...
    def fit(self):
        ...
        all_wrong_num_prompt_in_batch = 0
        all_correct_num_prompt_in_batch = 0
        num_gen_batches = 0
            ...
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    if not self.config.algorithm.filter_groups.enable:
                        ...
                        all_zero_uids, all_one_uids = (
                            [uid for uid, v in prompt_uid2metric_vals.items() if all(x == 0 for x in v)],
                            [uid for uid, v in prompt_uid2metric_vals.items() if all(x == 1 for x in v)],
                        )
                        ...
                        all_wrong_num_prompt_in_batch += len(all_zero_uids)
                        all_correct_num_prompt_in_batch += len(all_one_uids)
                    ...
                    if self.config.trainer.critic_warmup <= self.global_steps:
                        ...
                        print(actor_output.meta_info["metrics"])
                    ...
                    if rollout_data_dir:
                        with marked_timer("dump_rollout_generations", timing_raw, color="green"):
                            ...
                            response_length = batch.batch["response_mask"].sum(dim=-1).cpu().tolist()
                            scores = batch.batch["token_level_scores"].sum(-1).cpu().tolist()

                            advantages = batch.batch["advantages"]
                            max_response_length = batch.batch["responses"].shape[-1]
                            response_mask = batch.batch["attention_mask"][:, -max_response_length:].bool()
                            valid_adv_per_prompt = [
                                adv[m].mean().item() if m.any() else float('nan') for adv, m in zip(advantages, response_mask)
                            ]

                            reward_extra_infos_dict = {"response_length": response_length, "valid_adv_per_prompt": valid_adv_per_prompt}
                ...
                metrics["train/passed_num"] = num_prompt_in_batch
                metrics["train/all_wrong_num_prompt_in_batch"] = all_wrong_num_prompt_in_batch
                metrics["train/all_correct_num_prompt_in_batch"] = all_correct_num_prompt_in_batch
                metrics["train/gen_batch_size"] = self.config.data.gen_batch_size
                batch = None
                num_prompt_in_batch = 0
                num_gen_batches = 0
                all_wrong_num_prompt_in_batch = 0
